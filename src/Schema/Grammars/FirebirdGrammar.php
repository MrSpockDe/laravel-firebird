...
class FirebirdGrammar extends Grammar
{
    ...
    protected function modifyIncrement(Blueprint $blueprint, Fluent $column)
    {
        if (
            ...
            $mode = $column->generatedAs === true && $column->always ? 'ALWAYS' : 'BY DEFAULT';
            $sql = ' GENERATED '.$mode.' AS IDENTITY';
            $start = $column->get('startingValue', $column->get('from'));

            if (! is_null($start)) {
                $sql .= ' (START WITH '.(int) $start.')';
            }

            return $sql.($this->hasCommand($blueprint, 'primary') ? '' : ' PRIMARY KEY');
        }
    }
